...
const app = {
  ...
  async getAgentData(a_id) {
    ...
    let user =
      ...
      " GROUP BY state";
    let order =
      "select count(*) total,state from helplist " +
      (a_id ? waid : "") +
      "  GROUP BY state";
    let orderMonth =
      'select count(*) total,state from helplist where DATE_FORMAT( create_time, " % Y % m " ) = DATE_FORMAT(  CURDATE( ) ," % Y % m " ) ' +
      naid +
      "  GROUP BY state";
    let orderType =
      "select count(*) total,title from helplist " +
      (a_id ? waid : "") +
      "  GROUP BY title";
    let orderTypeMonth =
      'select count(*) total,title from helplist where DATE_FORMAT( create_time, " % Y % m " ) = DATE_FORMAT(  CURDATE( ) ," % Y % m " ) ' +
      naid +
      " GROUP BY title";
    let orderTotal =
    ...
    let turnoverDaily =
      ...
      naid;
    let turnoverMonth =
      'select sum(total_fee) turnover from helplist where state in (1,2,3) and is_pay=1 and DATE_FORMAT( create_time, " % Y % m " ) = DATE_FORMAT(  CURDATE( ) ," % Y % m " ) ' +
      naid;
    let turnoverYear =
      "select sum(total_fee) turnover from helplist where state in (1,2,3) and is_pay=1 and YEAR(create_time)=YEAR(NOW()) " +
      naid;
    let refund =
    ...
    let refundDaily =
      ...
      naid;
    let refundMonth =
      'select sum(total_fee) refund from helplist where state = 4 and is_pay=1 and DATE_FORMAT( create_time, " % Y % m " ) = DATE_FORMAT(  CURDATE( ) ," % Y % m " )  ' +
      naid;
    let refundYear =
      "select sum(total_fee) refund from helplist where state = 4 and is_pay=1 and YEAR(create_time)=YEAR(NOW()) " +
      naid;
    let data = {
