...
const app = {
    async add(args) {
        ...
        let sql = 'insert into cash_recode (uid,type,cash_fee,state,msg) value(?,?,?,?,?)'
        let params = [args.uid, args.type, args.cash_fee, args.state, args.msg]
        let result = await db.query(sql, params)
        return result
    },
    async cashsum(args) {
        let sql = 'select sum(cash_fee) sum from cash_recode where uid = ? and type =?'
