...
const app = {
    async complete(ctx) {
        ...
        if (bkdata.errno) {
            ...
            //获取服务项
            let server = (await hModel.getServerByDlIdAndServerName(y_user.pk_id, order.title))[0]
            console.log(server)
            //计算资金分流
            let p_get = parseFloat(server.p_sy) * parseFloat(order.total_fee)
            let a_get = parseFloat(server.dl_sy) * parseFloat(order.total_fee)
            let u_get = parseFloat(1 - server.p_sy - server.dl_sy) * parseFloat(order.total_fee)
            let data = {
                a_id: order.a_id,
                u_id: order.jd_id,
                h_id: order.id,
                ...
                a_get: a_get,
                u_get: u_get,
                rate: "p:" + server.p_sy + " d:" + server.dl_sy + " u:" + server.user_sy
            }
